...
class MTurk extends Model
{
  ...
  public static function testConnection()
  {
    try {
      ...
      Log::error('MTurk connection test failed', ['error' => $e->getMessage()]);
      return false;
    }
  }
  ...
  public function processAssignment($hit)
  {
    try {
      $mturk_hit = DB::table('mturk_hits')
        ->where('id', $hit->id)
      ...
      if (!$mturk_hit) {
        Log::error('MTurk hit not found', ['hit_id' => $hit->id]);
        return false;
      }
      ...
      Log::error('Error processing MTurk assignment', [
        'hit_id' => $hit->id,
        'error' => $e->getMessage()
      ]);
      return false;
  ...
  public function processBonus($hit)
  {
    try {
      $mturk_hit = DB::table('mturk_hits')
        ->where('id', $hit->id)
      ...
      Log::error('Error processing MTurk bonus', [
        'hit_id' => $hit->id,
        'error' => $e->getMessage()
      ]);
      return false;
  ...
  public function processQualification($hit)
  {
    try {
      $mturk_hit = DB::table('mturk_hits')
        ->where('id', $hit->id)
      ...
      DB::table('mturk_hits')
        ->where('id', $hit->id)
        ->update([
          'qualification_processed' => true,
          'updated_at' => now()
        ]);

      return true;
    } catch (\Exception $e) {
      Log::error('Error processing MTurk qualification', [
        'hit_id' => $hit->id,
        'error' => $e->getMessage()
      ]);
      return false;
